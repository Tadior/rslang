...
export class UsersLearnedWordsService {
  ...
  async getUserLearnWordsByUserId(userId: string): Promise<IUserLearnedWords> {
    const userLearnWords: UsersLearnedWordsEntity =
      await this.usersLearnedWordsEntityRepository.findOne({
        where: { userId: userId },
      });
    return userLearnWords.toResponse();
  }

  async getUserLearnWordsByWordId(
    userId: string,
    id: string,
  ): Promise<IUserLearnedWordsExists> {
    await this.usersService.getUserById(userId);
    const userLearnedWordsExists: IUserLearnedWords =
      await this.usersLearnedWordsEntityRepository
  ...
  async updateUserLearndedWords(
    ...
    const userLearnedWords: IUserLearnedWords =
      await this.getUserLearnWordsByUserId(userId);
