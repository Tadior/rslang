...
export class UsersWordsService {
  ...
  async createUserWords(
    ...
    const createdUserWords: UserWordsEntity =
      await this.userWordsRepository.create(newUserWord);
  ...
  async updateUserWords(
    ...
    await this.usersService.getUserById(userId);
    const userWords: IUserWords = await this.getUserWordsById(
      userId,
      wordId,
      true,
    );
