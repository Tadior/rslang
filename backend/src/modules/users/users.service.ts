...
export class UsersService {
    ...
    async createUser(createUserDto: CreateUserDto): Promise<IUser> {
        ...
        if (isUserExist) {
            throw new HttpException("User with this e-mail exists", HttpStatus.EXPECTATION_FAILED);
        }
    ...
    async getUserByLoginAndPassword(email: string, password: string): Promise<IUser> {
        const user: IUser = await this.userRepository.findOne({
