...
export class UsersSettingsService {
    ...
    async getUserSettings(userId: string): Promise<IUserSettings[]> {
        ...
        const userSettings: UserSettingsEntity[] = await this.userSettingsRepository.find({
            where: { userId: userId },
        });
        return userSettings.map((user) => user.toResponse());
    }

    async getUserSettingsById(userId: string, fullData?: boolean): Promise<IUserSettings> {
        await this.usersService.getUserById(userId);
        const userSettings: UserSettingsEntity = await this.userSettingsRepository.findOne({
            where: { userId: userId },
        });
        if (!userSettings) {
    ...
    async updateUserSettings(userId: string, updateUserSettingsDto: UpdateUserSettingsDto): Promise<IUserSettings> {
        await this.usersService.getUserById(userId);
